<?php
	...
	class fmakeFilms_genre extends fmakeCore{
		
		public $idField = "id_genre";
		public $table = "genres";
		public $table_notice_tags = "films_genre";
		...
		*/
		function addTag($name){
			$tag = $this->getWhere(array("`caption` = '{$name}'"));
			$tag = $tag[0];
			if($tag){
				return $tag;
			}
			$this -> addParam('caption',$name);
			$this -> newItem();
			$this -> params = array();
			return $this->getInfo();
		}
		...
		*/
		function addTags($tagsStr, $id_site_modul){
			...
			if($tagsNotDelete || !$tagsStr){
				...
				$delete	-> addTable($this->table_notice_tags) -> addWhere("`id_film`='".$id_site_modul."'") -> queryDB();
			}
		}
		/**
		*
		...
		*/
		function addNoticeTag($name,$id_site_modul){
			...
			$item = $this->getWhere(array("`id_film` = '{$id_site_modul}'","`{$this->idField}` = '{$tag[$this->idField]}'"));
			if($item){
				$this->table = $tmp;
				return $item[0];
			}
			...
			$this -> addParam("id_film",$id_site_modul);
			
			$this ->newItem();
			$item['id_film'] = $id_site_modul;
		...
		*/
		function getTags($id_site_modul){
			...
			$tmp = $this->table;
			$this->table = $this->table_notice_tags;
			$items = $this->getWhere(array("`id_film` = '{$id_site_modul}'"));
			$this->table = $tmp;
		...
		function delTagsPage($id_page){
			...
			$delete->addTable($this->table_notice_tags)->addWhere('id_film = '.$id_page)->queryDB();
		}
		
		function getTagsPress($id_tag){
			$select = $this->dataBase->SelectFromDB(__LINE__);
			$result = $this->dataBase->query("Select * from ".$this->table.",".$this->table_notice_tags." where ".$this->table.".".$this->idField." = ".$this->table_notice_tags.".".$this->idField." AND  id_film in (Select id_film from ".$this->table_notice_tags." where ".$this->idField." = ".$id_tag." ) ", __LINE__);
