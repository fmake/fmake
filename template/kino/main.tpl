						...
						<div class="film-item content-block">
							<table><tr>
								<td class="left-film" width="100%" >
									<div class="header"><span class="date" >{df('date','d.m.Y',item.date)}</span><a href="/{item.id}-{item.url}/">{item.caption}</a></div>
									<p class="janr" ><span><a href="/{item.id}-{item.url}/#comment" ><img src="/images/icon-comment.gif" /> Ваш отзыв</a></span>
									[[if item.genres]]
										Жанр фильма: 
									...
									<a href="/{item.id}-{item.url}/#view-online" class="black-submit" title="Смотреть онлайн">СМОТРЕТЬ ОНЛАЙН</a>
									<a href="/{item.id}-{item.url}/#view-online" class="orange-submit" title="Скачать" >СКАЧАТЬ</a>
								</td>
								<td class="right-film">
										...
										<script>
											$(function(){  
												...
												[[if item.star_read]]
												[[raw]]
													$('form#form[[endraw]]{item.id}[[raw]] :radio.star').rating([[endraw]][[if item.star_count]]'select',{item.star_count-1}[[endif]][[raw]]);
													$('form#form[[endraw]]{item.id}[[raw]] :radio.star').rating('readOnly',true);
												[[endraw]]
												[[else]]
												[[raw]] 
													$('form#form[[endraw]]{item.id}[[raw]] :radio.star').rating([[endraw]][[if item.star_count]]'select',{item.star_count-1}[[endif]][[raw]]);
													$('form#form[[endraw]]{item.id}[[raw]] :radio.star').rating('readOnly',false);
												[[endraw]]
												[[endif]]
												[[raw]]
