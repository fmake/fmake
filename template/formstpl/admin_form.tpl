<FORM name='{$form->formName}' action='{$form->formAction}' method='{$form->formMethod}' enctype='{$form->formEnctype}'>
	<table class="edit-forms" >
		<h1>{$form->formLegend}</h1>
		<?
		foreach ($form->elements as $id=>$elements){
			switch($elements->type){
				case "NEWTEXT":?>
					<tr>
						<td class="mini" ><span><b><?=$elements->caption;?></b></span><br /><?=$elements->text?></td>
						<td class="editor-max" ><?=$elements->value?></td>
					</tr>
					<?break;
				...
				break;
				default:
					if($request->dop_polya && $elements->name=='title') $dop_polya = '';
					else if($request->dop_polya && $elements->name=='tags') $dop_polya = '';
					else if($request->dop_polya) $dop_polya = ' style="display:none;"';
					else $dop_polya = '';
					$input = "<";
					$input .= "INPUT type='".$elements->type."' ";
					$input .= "name='".$elements->name."' ";
