...
[[ macro childGenerateItem(item,actions,request) ]]
	<div>							
		...
		<a href="[[if item['delete_security']]]/admin/?modul={item.modul_redir}[[else]]/admin/?modul={request.modul}&id={item.id}&dop_polya=hide&action=edit[[endif]]" class="droppable_inner[[if item['delete_security']]] delete_security[[endif]]" >{item['caption']}[[if item.date]]({df('date','d.m.Y',item.date)})[[endif]]</a>
		<div class="actions" >
